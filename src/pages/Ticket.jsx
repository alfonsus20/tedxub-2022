...
const Ticket = () => {
  ...
  const handleCheckout = async () => {
    try {
      ...
      for (let item of data.data) {
        if (
          item.jenis_tiket == selectedTicket.jenis_tiket &&
          item.remaining_ticket < quantity
        ) {
          setAlertStatus(`Maaf, tiket tersisa ${item.remaining_ticket} lagi.`);
          handleOpenAlert();
        } else if (
          item.jenis_tiket == selectedTicket?.jenis_tiket &&
          item.remaining_ticket == 0
        ) {
          ...
          return navigate("/ticket-form", {
            state: {
              type: selectedTicket?.jenis_tiket,
              price: selectedTicket?.price,
              remaining_ticket: selectedTicket?.remaining_ticket,
              quantity: quantity,
            },
          });
        }
  ...
  const ticketAvailable = false;

  return (
    <div
      className="ticket-container bg-cover bg-no-repeat bg-[#1D1B21]"
    ...
    >
      ...
      <Spinner
        isOpenSpinner={isOpenSpinner}
        onCloseSpinner={handleCloseSpinner}
      />
      <div className="m-auto z-10">
        ...
        <div className="w-full">
          {activeTicket && allTicket && (
            <TicketCarousel
              activeTicket={activeTicket}
              setSelectedTicket={setSelectedTicket}
              ticketLists={allTicket}
            />
          )}
        </div>
        ...
        <div className="flex flex-col font-jakarta text-center text-main-2 gap-2 mt-10">
          ...
          <p className="font-jakartaBold text-4xl">
          ...
          <div className="flex flex-row flex-wrap justify-center items-center mt-5 gap-5">
            <div className="flex flex-row items-center justify-between w-40 bg-main-2 rounded-full px-5 py-2 text-main-1 font-jakartaBold">
              <button
                className="px-2"
                disabled={
                  selectedTicket?.jenis_tiket == activeTicket?.jenis_tiket &&
                  selectedTicket?.remaining_ticket > 0
                    ? false
                    : true
                }
            ...
            <button
              ...
              disabled={ticketAvailable ? false : true}
              className={`w-40 px-5 py-2 font-jakartaBold text-main-2 rounded-full ${
                ticketAvailable
                  ? "bg-main-3 hover:bg-main-2 hover:text-main-3 duration-200"
          ...
          {ticketAvailable && (
            <p className="mt-5">
              ...
              <span className="ml-5 font-jakartaBold">
                Rp {quantity * selectedTicket?.price}
              </span>
            </p>
          )}
        </div>
